...
function LaureatAccepted({ LaureatAccepted }) {
                ...
                post(
                    route('laureat.accepted.block', { LaureatChosed: LaureatId }),
                    {
                        ...
                        onSuccess: () => {
                            ...
                            toast.success(t('Gestionnaire.laureat.block_success'));
                        }
                    }
                )
                :
    ...
    const EmptyState = () => (
        <div className="flex flex-col items-center justify-center min-h-[400px] rounded-lg">
            ...
            <h3 className="text-xl font-medium text-gray-900 mb-1">{t('Gestionnaire.laureat.no_laureates')}</h3>
            <p className="text-gray-500">{t('Gestionnaire.laureat.no_accepted_available')}</p>
        </div>
    );
        ...
        if (auth.user.valide) {
            if (confirm(t('Gestionnaire.laureat.delete_confirm'))) {
                post(route('gestionnaire.account.delete', id), {
                    ...
                    onSuccess: () => {
                        setData({
                            LaureatAccepted: data.LaureatAccepted.filter((Laureat) => Laureat.id !== id)
                        });
            ...
            <div className="max-w-7xl mx-auto">
                <div className="mb-8">
                    <h1 className="text-3xl font-bold text-gray-900 mb-2">
                        {t('Gestionnaire.laureat.accepted_laureates')} ({data.LaureatAccepted.length})
                    </h1>
                    <p className="text-gray-600">{t('Gestionnaire.laureat.manage_view')}</p>
                </div>
                                            ...
                                            <tr key={laureate.id} className="group hover:bg-gray-50 transition-colors">
                                                ...
                                                <td className="px-4 py-4 text-left">
                                                    <button
                                                        ...
                                                        className="inline-flex mx-1 items-center gap-2 px-4 py-2 bg-red-50 text-red-700 rounded-lg font-medium text-sm  transition-colors hover:bg-red-600 hover:text-white"
                                                    >
                                                        <Ban className="w-4 h-4" />
                                                        {t('Gestionnaire.laureat.block_laureat')}
                                                    </button>
...
LaureatAccepted.layout = page => <Dashboard children={page} title={t('Gestionnaire.laureat.accepted_title')} />;
